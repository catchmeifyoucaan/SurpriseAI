...
const HtmlBulkSenderPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!campaignData.campaignName || !campaignData.subject || !campaignData.htmlBody || (!campaignData.recipientsManual && !campaignData.recipientFile) || !campaignData.selectedSmtp) {
        setFormMessage("Error: Please fill all required fields including Campaign Name, Subject, HTML Body, Recipients, and select an SMTP Configuration.");
        setIsSending(false);
        return;
    }

    console.log('HTML Bulk Email Campaign Data:', campaignData);
    const activityDescription = `Submitted HTML Bulk Email campaign: ${campaignData.campaignName} via SMTP ID ${campaignData.selectedSmtp}`;
    if (auth.user) auth.logUserActivity(auth.user.id, activityDescription);
    
    setTimeout(() => {
      setIsSending(false);
      setFormMessage(`Campaign "${campaignData.campaignName}" logged for processing by backend via SMTP ID ${campaignData.selectedSmtp}.`);
      ...
      setCampaignData({ campaignName: '', subject: '', htmlBody: '<p>Start new email...</p>', recipientsManual: '', recipientFile: null, selectedSmtp: auth.smtpConfigurations[0]?.id || '', generatedByAI: false });
  ...
  return (
    <div className="bg-secondary p-4 sm:p-6 rounded-lg shadow-xl min-h-full">
      ...
      {formMessage && (
        <div className={`p-3 rounded-md text-sm mb-6 ${formMessage.toLowerCase().includes("error") ? 'bg-red-900 text-red-100' : formMessage.toLowerCase().includes("ai is generating") || isLoadingAiHtml ? 'bg-sky-800 text-sky-100' : 'bg-green-800 text-green-100'}`}>
          {formMessage}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <form onSubmit={handleSubmit} className="space-y-6 p-4 sm:p-6 bg-primary rounded-lg shadow-2xl border border-slate-700">
          ...
          <div className="relative">
            ...
            {isAiAvailable() && (
                <Button
                    ...
                    className="absolute right-1 bottom-1 text-accent p-1"
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
                <label htmlFor="htmlBody" className="block text-sm font-medium text-text-secondary">
                    HTML Email Body
                </label>
                {isAiAvailable() && (
                    <Button
          ...
          <div>
            <Select
              ...
              value={campaignData.selectedSmtp}
              onChange={handleChange}
              required
          ...
          {!isAiAvailable() && (
            <p className="text-xs text-amber-400 text-center mt-2">AI suggestions are disabled. Configure API_KEY to enable them.</p>
          )}
        ...
        <div className="bg-primary p-4 sm:p-6 rounded-lg shadow-2xl border border-slate-700">
          ...
          <div 
            className="w-full h-[600px] border border-slate-600 rounded bg-white text-black overflow-auto p-2"
            dangerouslySetInnerHTML={{ __html: htmlPreview }}
          ></div>
          <p className="text-xs text-text-secondary mt-2">Note: Preview is rendered by the browser and may differ slightly from email clients.</p>
        </div>
      </div>
                ...
                <tbody className="bg-primary divide-y divide-slate-700">
                    {campaigns.map(campaign => (
                        <tr key={campaign.id} className="hover:bg-slate-700/30 transition-colors">
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-text-primary">{campaign.name} {campaign.generatedByAI && <SparklesIcon className="w-3 h-3 inline-block ml-1 text-accent" />}</td>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <Button variant="ghost" size="sm" onClick={() => { if(auth.user) auth.logUserActivity(auth.user.id, `Viewed details for HTML campaign: ${campaign.name}`); alert(`Viewing details for ${campaign.name} - not fully implemented.`);}} className="mr-2 !p-1.5">View Details</Button>
                            </td>
                        </tr>
                    ))}
                    {campaigns.length === 0 && (
                        <tr>
                            <td colSpan={5} className="px-6 py-4 text-center text-text-secondary">No HTML campaigns logged yet.</td>
                        </tr>
                    )}
                </tbody>
            </table>
